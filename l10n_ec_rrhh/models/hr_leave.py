...
class HrLeave(models.Model):
    ...
    # Bandera para mostrar sección de vacaciones en la vista (más robusto que usar dot-path en invisible)
    ...
    def _compute_vacation_counters(self):
        ...
        #Tolerante si el modelo aún no existe
        ...
        for r in self:
            ...
            # Disponibles: si hay vacation.period úsalo; si no, intenta un fallback
            ...
            if VacationPeriodModel:
                ...
                # Fallback: usa un campo tuyo de contrato si lo tienes (ajusta el nombre),
                # o deja 0 hasta que instales el módulo que define vacation.period.
                total_disponibles = float(getattr(contract, 'l10n_ec_ptb_total_credited_vacations', 0.0))

            # Tomadas: leaves aprobadas de tipo vacaciones en el rango del contrato
            dom_leaves = [
                ('employee_id', '=', r.employee_id.id),
    ...
    def _onchange_block_exceeding_vacation(self):
        ...
        for r in self:
            if not r.is_vacation_selected:
                continue
            # Asegurar que los contadores ya estén calculados en el onchange
            r._compute_vacation_counters()
            # Si pide más que los restantes, avisar
            # number_of_days puede ser float; comparamos con margen pequeño si quieres
            if r.number_of_days and r.vacation_remaining and (r.number_of_days > r.vacation_remaining):
                # Mensaje claro pero no rompe UI (solo advertencia)
                return {
                    'warning': {
        ...
        for r in self:
            if r.is_vacation_selected and r.state in ('validate', 'validate1'):
                ...
                if r.number_of_days > (r.vacation_remaining + r.number_of_days_taken_by_this_request()):
                    # nota: number_of_days_taken_by_this_request() es opcional si quieres
                    # descontar el propio registro en “tomadas”; aquí ejemplo directo:
    ...
    # --- UTILIDAD: días solicitados como número positivo (según policy de cómputo) ---
    def _get_requested_days(self):
        # Odoo ya computa number_of_days (o request_unit_half…) según política.
        # Usamos el campo estándar 'number_of_days'
        self.ensure_one()
        return abs(self.number_of_days or 0.0)

    # --- VALIDACIÓN: no exceder disponible si es vacaciones ---
        ...
        for rec in self.filtered(lambda l: l.holiday_status_id.is_vacation and l.state == "validate"):
            ...
            for bal in balances:
                ...
                if take > 0:
                    ...
                    remaining -= take
            if remaining > 1e-6:
                # no debería pasar por la constraint, pero por si hay carreras
